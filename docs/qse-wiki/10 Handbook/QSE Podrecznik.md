...
> 💡 **Ciekawostka:** Po kalibracji empirycznej waga modularity w AGQ wynosi **0.0**. Oznacza to, że na zbiorze kalibracyjnym 74 projektów OSS-Python (z 80) modularity nie wnosiła unikalnego sygnału, gdy pozostałe trzy metryki były obecne. To nie znaczy, że jest bez wartości diagnostycznej - ale jako predyktor procesu nie różnicowała niezależnie od reszty. Kalibracja per-język jest planowanym kierunkiem badań.
...
**Problem który rozwiązuje:** AGQ=0.65 to dobry wynik dla Javy (powyżej średniej 0.627) i słaby dla Go (poniżej średniej 0.815). Surowa liczba bez kontekstu języka jest myląca.
...
| ✅ Mówi że "coś się zmieniło na gorsze" | ❌ Nie wskazuje automatycznie konkretnego pliku do naprawy (ale `qse discover` identyfikuje naruszenia granic modułowych) |
...
|---|---|---|
| AGQ Core (4 metryki) | ✅ Zaimplementowany, 149 testów | `qse/graph_metrics.py` |
| AGQ Enhanced (5 metryk) | ✅ Zaimplementowany | `qse/agq_enhanced.py` |
| Skaner Python/Java/Go (Rust, jedyny) | ✅ Działający, 7–46× szybszy | `qse-core/` |
...
Średnia = (0.62 + 0.55 + 0.71 + 0.63 + 0.58 + ...) / 79 = 0.627
```
...
Java: mean=0.627, std=0.096  →  projekty Java bardziej zróżnicowane (większe rozproszenie)
```
...
Java: 0.627 ± 0.096  →  większość projektów Java między 0.53 a 0.72
```
